  ...
  App.Model.ChartWidgetModel = Backbone.Model.extend({
    ...
    initialize: function(options) {
      ...
      if (this.options.compareIndicators) {
        this.compareIndicatorsModels = this.options.compareIndicators.map(
    ...
    _fetchAnalysisPartials: function() {
      ...
      return $.when.apply(
        ...
        partialIndicatorModels.map(function(partialIndicatorModel) {
          ...
          partialIndicatorModel
            .fetch()
            .done(function() {
              deferred.resolve(partialIndicatorModel.toJSON());
            })
      ...
      res.data = analysisColumns
        .map(function(columnName, index) {
          ...
          return partialIndicators[index].data.map(function(row) {
            ...
            row.groupPercentage = _.findWhere(
              this.analysisIndicatorModel.get("data"),
              { label: columnName }
            ).percentage;
            return row;
          }, this);
    ...
    _isJurisdictionCompare: function() {
      return this.compareIndicatorsModels.reduce(function(
        res,
        comparePartialModel
      ) {
        return (
          (comparePartialModel.options.filters &&
            comparePartialModel.options.filters.length &&
            !!_.findWhere(comparePartialModel.options.filters, {
              id: "jurisdiction"
            })) ||
          res
        );
    ...
    _getJurisdictionName: function(comparePartialModel) {
      var jurisdictionFilter = _.findWhere(
        comparePartialModel.options.filters,
        { id: "jurisdiction" }
      );
    ...
    _getCompareGroupName: function(model) {
      if (this._isJurisdictionCompare()) {
        ...
        return model !== this.indicatorModel
          ...
          : jurisdictionFilter
          ? jurisdictionFilter.options[0]
          : "All jurisdictions";
      } else {
        return (
          App.Helper.Indicators.COUNTRIES[model.options.iso] +
          " " +
          model.options.year
        );
      }
    ...
    _fetchComparePartials: function() {
      return $.when.apply(
        ...
        this.compareIndicatorsModels.map(function(compareIndicatorsModel) {
          ...
          // data to be parsed
          var deferred = $.Deferred();
          compareIndicatorsModel
            .fetch()
            .done(function() {
              deferred.resolve(compareIndicatorsModel.toJSON());
            })
            .fail(deferred.reject);
          return deferred;
        })
    ...
    _joinComparePartials: function() {
      ...
      var partialModels = [this.indicatorModel].concat(
        this.compareIndicatorsModels
      );
      ...
      if (!this._isJurisdictionCompare()) {
        partialModels.sort(function(partialModalA, partialModelB) {
          if (partialModalA.options.year > partialModelB.options.year)
            return -1;
      ...
      res.data = partialModels
        .map(function(partialModel) {
          return partialModel.get("data").map(function(row) {
            return _.extend({}, row, {
              group: this._getCompareGroupName(partialModel)
            });
          }, this);
        }, this)
        .reduce(function(res, data) {
    ...
    fetch: function() {
      ...
      if (this.analysisIndicatorModel) {
        ...
        this.fspMapsModel
          .fetch({ data: this.options.body, type: "POST" })

module Dpapi
  class GdpController < ApiController
    ...
    def by_country_over_time
      gdp_data = GrossDomesticProductOverTime.joins('LEFT JOIN countries ON countries.iso = gross_domestic_product_over_times.iso')
        .select('*, gross_domestic_product_over_times.id as id, countries.name as country, gross_domestic_product_over_times.iso')
        .find_by(iso: params[:iso])
